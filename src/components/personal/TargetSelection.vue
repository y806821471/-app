  ...
  <div class="TargetSelection">
    ...
    <div v-for="(item,index) in list" :key="item.id">
      <div v-if="answerIndex == index+1">
        ...
        <!-- 单选框 -->
        <div v-if="item.check_type == 1" class="content">
...
export default {
  ...
  watch: {
    answerIndex(newV, oldV) {
      // this.questionId.push(this.list[this.answerIndex - 1].id);
      // console.log("问题ID", this.questionId);
    ...
    submit() {
      this.list.forEach((i, index) => {
        if (this.answerIndex == index + 1) {
          ...
          } else if (i.check_type == 3) {
            this.answerId = this.result.join(",");
          }
        }
      });
      if (this.answerId == "") {
